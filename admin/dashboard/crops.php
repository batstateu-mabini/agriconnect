...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['approve_id'])) {
        ...
        $stmt = $pdo->prepare("UPDATE crop_requests SET status = 'approved' WHERE id = ?");
    ...
    if (isset($_POST['schedule_id'])) {
        ...
        $stmt = $pdo->prepare("UPDATE crop_requests SET schedule_date = ?, schedule_time = ? WHERE id = ?");
        $stmt->execute([$schedule_date, $schedule_time, $schedule_id]);

        // Get user phone and send SMS
        $stmt = $pdo->prepare("SELECT cr.*, u.phone, u.first_name FROM crop_requests cr LEFT JOIN users u ON cr.user_id = u.id WHERE cr.id = ?");
        $stmt->execute([$schedule_id]);
        $req = $stmt->fetch();
        if ($req && $req['phone']) {
            $recipient = $req['phone'];
            $formatted_date = date('F j, Y', strtotime($schedule_date));
            $message = "Magandang araw, ang iyong crop request ay nakatakdang i-pickup sa {$formatted_date}, sa ganitong oras {$schedule_time}";
            $sender_id = 'PhilSMS';
    ...
    if (isset($_POST['complete_id'])) {
        ...
        $stmt = $pdo->prepare("UPDATE crop_requests SET status = 'completed' WHERE id = ?");
        $stmt->execute([$complete_id]);
    }
}

// Fetch pending crop requests
$stmt = $pdo->query("SELECT cr.*, u.first_name, u.last_name, u.email FROM crop_requests cr JOIN users u ON cr.user_id = u.id WHERE cr.status = 'pending' ORDER BY cr.id DESC");
$pending_requests = $stmt->fetchAll();


// Approved requests (no schedule set)
$stmt = $pdo->query("SELECT cr.*, u.first_name, u.last_name, u.email FROM crop_requests cr JOIN users u ON cr.user_id = u.id WHERE cr.status = 'approved' AND (cr.schedule_date IS NULL OR cr.schedule_date = '') ORDER BY cr.id DESC");
$approved_requests = $stmt->fetchAll();

// Waiting for pickup (approved, schedule set)
$stmt = $pdo->query("SELECT cr.*, u.first_name, u.last_name, u.email FROM crop_requests cr JOIN users u ON cr.user_id = u.id WHERE cr.status = 'approved' AND cr.schedule_date IS NOT NULL AND cr.schedule_date != '' ORDER BY cr.id DESC");
$waiting_requests = $stmt->fetchAll();

// Completed crop requests
$stmt = $pdo->query("SELECT cr.*, u.first_name, u.last_name, u.email FROM crop_requests cr JOIN users u ON cr.user_id = u.id WHERE cr.status = 'completed' ORDER BY cr.id DESC");
...
<div class="row">
    ...
    <nav class="sidebar">
        ...
        <ul class="nav flex-column">
            ...
            <li class="nav-item"><a class="nav-link active" href="#">🌾 Crop Requests</a></li>
    ...
    <main class="main-content">
        ...
        <!-- Pending Crop Requests Table -->
        <div class="card shadow-sm mb-4">
            <div class="card-header bg-warning text-dark">Pending Crop Requests</div>
                ...
                <div class="table-responsive">
                    <table class="table table-striped mb-0">
                        ...
                        <?php if (count($pending_requests) > 0): ?>
                                                                                    ...
                                                                                    <div class="modal-dialog modal-dialog-centered">
                                                                                        <div class="modal-content">
                                                                                            ...
                                                                                            <div class="modal-body">
                                                                                                Are you sure you want to approve this crop request?
                                                                                            </div>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr><td colspan="6" class="text-center text-muted">No pending crop requests.</td></tr>
                        <?php endif; ?>
                        </tbody>
                    </table>
                </div>
        ...
        <!-- Approved Crop Requests Table -->
        <div class="card shadow-sm mb-4">
            <div class="card-header bg-info text-dark">Approved Crop Requests</div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-striped mb-0">
                        <thead>
                        ...
                        <?php if (count($approved_requests) > 0): ?>
                                            ...
                                            document.addEventListener('DOMContentLoaded', function() {
                                                ...
                                                function updateSmsPreview() {
                                                    ...
                                                    if (date && time) {
                                                        ...
                                                        smsPreview.textContent = `Magandang araw, ang iyong crop request ay nakatakda sa ${formattedDate}, sa ganitong oras ${formattedTime}`;
                                                    } else {
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr><td colspan="6" class="text-center text-muted">No approved crop requests.</td></tr>
                        <?php endif; ?>
                        </tbody>
                    </table>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-striped mb-0">
                        ...
                        <?php if (count($waiting_requests) > 0): ?>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr><td colspan="8" class="text-center text-muted">No waiting crop requests.</td></tr>
                        <?php endif; ?>
                        </tbody>
                    </table>
                </div>
        ...
        <?php
        $today = date('Y-m-d');
        $reminder_requests = $pdo->query("SELECT cr.*, u.first_name, u.last_name, u.email, cr.schedule_time, cr.crops, cr.notes FROM crop_requests cr JOIN users u ON cr.user_id = u.id WHERE cr.status = 'approved' AND cr.schedule_date = '$today' ORDER BY cr.schedule_time ASC")->fetchAll();
        ?>
        <div class="card shadow-sm mb-4">
        ...
        <!-- Completed Crop Requests Table -->
        <div class="card shadow-sm mb-4">
            <div class="card-header bg-success text-white">Completed Crop Requests</div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-striped mb-0">
                        <thead>
                        ...
                        <?php if (count($completed_requests) > 0): ?>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr><td colspan="5" class="text-center text-muted">No completed crop requests.</td></tr>
                        <?php endif; ?>
                        </tbody>
                    </table>
                </div>
